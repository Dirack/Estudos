...
\preambulo{Projeto de tese apresentado ao Programa de Pós-Graduação em Geofísica do Instituto de Geociências 
...
So, Padé CRS traveltime aproximations can be used in CRE gather interpolation, in order to provide information data 
and allow CRE stacking, despite convencional CMP gather as input.
\vspace{\onelineskip} 
\noindent 
\par Keywords: Padé approximants. Non-hyperbolic traveltime aproximation. CRS Padé traveltime aproximation interpolation.
\end{resumo}
...
\mainmatter
 ...
 \include{objetivo} % OBJETIVOS
 \include{justificativa} %
 \include{estado} % Estado da arte do problema
 \include{metodologia} %
 \include{resultados} %
 \include{cronograma} %
 \include{conclusao} %
...
% % \include{cap4} %CRE
%  \include{cap7} %CRE
% \include{cap5} %CRS JC/PEDRO CHIRA
%  \include{cap6} %ESTUDO REFLETOR CIRCULAR
