...
var viewer = new Cesium.Viewer('cesiumContainer', {
  ...
  var getSelectorLocation = new Cesium.CallbackProperty(function getSelectorLocation(time, result) {
    ...
    screenSpaceEventHandler.setInputAction(function endClickShift() {
      ...
      console.log(Cesium.BoundingRectangle.fromPoints(packed).intersect(polygono))

      let result = Cesium.BoundingRectangle.intersect(Cesium.BoundingRectangle.fromPoints(packed), polygono)

      if(result == 0){
        myhorizon._polygon.material.color = Cesium.Color.BLUE
      }else{
        myhorizon._polygon.material.color = Cesium.Color.RED
      }
  ...
  viewer._toolbar.appendChild(test)
